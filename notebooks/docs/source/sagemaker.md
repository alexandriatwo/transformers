...
* [Prepare a training script](sagemaker.md#prepare-a-transformers-fine-tuning-script)
* [Create a `HuggingFace` Estimator](sagemaker.md#create-an-huggingface-estimator)
* [Run training by calling the `fit` method](sagemaker.md#execute-training)
* [Access you model](sagemaker.md#access-trained-model)
...
Later we define `hyperparameters` in the [HuggingFace Estimator](sagemaker.md#create-an-huggingface-estimator), which are passed in as named arguments and and can be processed with the `ArgumentParser()`.
...
_Note: Unless your training job completes quickly, we recommend you use_ [_checkpointing_](https://docs.aws.amazon.com/sagemaker/latest/dg/model-checkpoints.html) _with managed spot training, therefore you need to define the `checkpoint_s3_uri`._
